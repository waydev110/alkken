<?php
if(isset($_POST['start']) && isset($_POST['id_plan'])){
    require_once '../../../helper/all_member.php';    
    require_once '../../../model/classBonusPasangan.php';
    $obj = new classBonusPasangan();
    ...
    $result = $obj->riwayat_poin_pasangan($id_member, $posisi, $status, $type, $start, $id_plan);
    if($result){
        $html = '';
        ...
        if($count > 0){
            ...
            while($row = $result->fetch_object()){
                ...
                $status_poin = status_poin($row->status);
                ...
                $html .= '<div class="card mb-0 rounded-0 border-0 border-bottom bonus-item '.$row->type.'">
                    <div class="card-body">
                        <div class="row">
                            ...
                            <p class="mb-0 text-theme size-12"> <strong><i>'.ucfirst($row->posisi).'</i></strong> '.poin($row->poin).' dari '.jenis_aktivasi($row->jenis_aktivasi).'</p>
                                <p>
                                    <span class="text-default fw-semibold mb-1 size-14">
                                        '.$row->nama_samaran.' ('.$row->id_member.')
                                    </span>
                                </p>
                            </div>
