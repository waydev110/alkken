<?php
    ...
    require_once '../../../model/classBonusMatchingReward.php';
    require_once '../../../model/classSMS.php';

    $cm = new classMember();
    $cbro = new classBonusRewardSetting();
    $obj = new classBonusReward();
    $cbmr = new classBonusMatchingReward();
    $sms  = new classSMS();
    ...
    try {
        ...
        if($create){        
            ...
            if($set_bonus->id_plan == 8 && $sponsor_peringkat >= 2){
                $keterangan = 'Matching Reward '.$set_bonus->nama_plan.' dari ( ID: '.$id_member.' / '.$nama_samaran.' / '.$user_member.' )';
                $cbmr->set_id_member($sponsor_id);
                $cbmr->set_nominal($nominal);
                $cbmr->set_reward($reward);
                $cbmr->set_poin($poin);
                $cbmr->set_id_bonus_reward_setting($id_bonus_reward_setting);
                $cbmr->set_dari_member($member_id);
                $cbmr->set_status_transfer('0');
                $cbmr->set_keterangan($keterangan);   
                $cbmr->set_created_at($created_at);        
        
                $create_matching = $cbmr->create();
                if($create_matching){
                    $sms->smsKlaimReward($sponsor_id, $nominal, $keterangan, $created_at);
                }
            }
        }
    } catch (\Exception $e) {
