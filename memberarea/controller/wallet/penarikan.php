<?php
    ...
    $maksimal_penarikan = $plan->maximal_wd-$total_wd;
    $percent_admin = $plan->persentase_admin;
    $admin = $maksimal_penarikan * $percent_admin/100;
    
    if($jumlah < $minimal_penarikan ){
        echo json_encode(['status' => false, 'message' => 'Minimal penarikan : .'.rp($minimal_penarikan)]);
        return false;
    }

    if($jumlah > $maksimal_penarikan ){
        echo json_encode(['status' => false, 'message' => 'Maksimal penarikan : .'.rp($maksimal_penarikan)]);
        return false;
    }
    
    if ($jumlah % 10000 !== 0) {
        echo json_encode(['status' => false, 'message' => 'Jumlah penarikan harus dalam kelipatan 10.000']);
        return false;
    }

    $saldo = $cw->saldo($session_member_id, 'cash');
    if($jumlah > $saldo ){
        echo json_encode(['status' => false, 'message' => 'Saldo tidak cukup. Maksimal penarikan : '.rp($saldo)]);
        return false;
    }
    ...
    try {
        ...
        $keterangan = 'Penarikan Wallet Cash';    
