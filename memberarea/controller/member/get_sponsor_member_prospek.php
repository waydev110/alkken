<?php  
    ...
    $member_prospek = $cmp->show_member($sponsor);
    $sponsor = $cm->show_id_by_user($member_prospek->sponsor);
    if(!$sponsor){
        echo json_encode(['status' => false, 'message' => 'Data Tidak ditemukan.']);
        return false;
    }
    $upline = base64_decode(addslashes(strip_tags($_POST['upline'])));
    $data_sponsor = $cm->data_sponsor($session_member_id, $sponsor->id, $upline);
    if(!$data_sponsor){
