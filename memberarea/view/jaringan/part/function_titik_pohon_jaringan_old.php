...
function titik($id, $upline, $action, $next = '0', $_binary = true)
{
    ...
    if ($member == null) {
        ...
        $action[$next][2] = false;
?>
        <div class="card-member member-blank bgcolor-<?= $bgcolor ?>">
            <?= $link ?>
            <div class="card-member-body pt-2">
                ...
                <div class="member-info">
                    <span>0</span>
                    <span><i class="fas fa-users-class"></i></span>
                    <span>0</span>
                </div>
                <div class="member-info">
                    <span>0</span>
                    <span><i class="fas fa-a"></i></span>
                    <span>0</span>
                </div>
                <?php
                // if($_binary == true) { 
                ...
                ?>
                <div class="member-profile px-3" style="height:27px">
                </div>
                <div class="member-profile">
    ...
    ?>
        ...
        <div class="card-member bgcolor-<?= $member->bg_color ?>">
            <a href="?go=genealogy_v1&id_member=<?= $member->id_member ?>"><img src="../images/plan/<?= $member->icon_plan ?>" alt="" width="100%"></a>
            <div class="card-member-body pt-2">
                <div class="member-profile">
                    <?php
                    // echo $reposisi
                ...
                <?php
                $poin_pasangan = $cm->total_poin_pasangan($id, 4);
                ?>
                <div class="member-info">
                    <span><?= currency($poin_pasangan->jumlah_kiri) ?></span>
                    <span data-toggle="tooltip" data-placement="top" title="Total Poin Pasangan"><i class="fas fa-a"></i></span>
                ...
                <?php if ($_binary == true) {
                    while ($row = $plan_pasangan->fetch_object()) {
                        $kondisi = false;
                        if ($row->syarat_ro == 1) {
                            $jumlah_poin_ro = $cm->jumlah_poin_ro($member->id, 8);
                            if ($jumlah_poin_ro > 0) {
                                $kondisi = true;
                            } else {
                                $kondisi = false;
                        ...
                        $poin_pasangan = $cm->jumlah_poin_pasangan($id, $row->id);
                        if ($kondisi == true) {
                ?>
                            <div class="member-info">
                                <span><?= currency($poin_pasangan->potensi_kiri) ?></span>
                                <span data-toggle="tooltip" data-placement="top" title="Poin Pasangan <?= $row->show_name ?>"><i class="fas fa-people-arrows"></i></span>
                        ...
                        ?>
                            <div class="member-info">
                                <span><?= currency($poin_pasangan->potensi_kiri) ?></span>
                                <span data-toggle="tooltip" data-placement="top" title="Poin Pasangan <?= $row->show_name ?>"><i class="fas fa-lock"></i></span>
                                <span><?= currency($poin_pasangan->potensi_kanan) ?></span>
                            </div>
                <?php
                        }
                ...
                ?>
                            <div class="member-info">
                                ...
                                <span data-toggle="tooltip" data-placement="top" title="Poin Reward <?= $row->nama_reward ?>"><i class="fas fa-<?=$row->icon_reward?>"></i></span>
                        ...
                        ?>
                            <div class="member-info">
                                <span>0</span>
                                <span data-toggle="tooltip" data-placement="top" title="Poin Reward <?= $row->nama_reward ?> Terkunci"><i class="fas fa-lock"></i></span>
                                <span>0</span>
                            </div>
                <?php
                        }
                ...
                <div class="member-profile px-3">
                    <a href="<?= site_url('posting_ro') ?>&id_member=<?= base64_encode($member->id) ?>" class="btn btn-sm btn-dark size-12 rounded-pill">Posting Fast Track</a>
                </div>
                <div class="member-profile">
                    <h3 class="member-rank"></h3>
                </div>
