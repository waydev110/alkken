                ...
                ?>
                    <div class="card mb-0 rounded-0 border-0 border-bottom bonus-item status<?=$row->status?>">
                        <div class="card-body">
                            <div class="row">
                                <div class="col align-self-center">
                                    <a class="text-dark size-12" href="?go=klaim_autosave_detail&id_order=<?=base64_encode($row->id)?>" class="size-11">Tanggal : <?=tgl_indo_jam($tanggal_order)?></a>
                                    <p class="text-default fw-bold mb-0 size-18">
                                        <?=currency($row->nominal+$row->saldo_poin)?>
                                    </p>
                                    <?php if($row->nominal > 0){ ?>
                                        <p class="text-danger fw-bold mb-0 size-12">
                                            Total Bayar : <?=currency($row->nominal)?>
                                        </p>
                                    <?php
                                            }
                                    ?>
                                ...
                                <div class="col-auto align-self-right">
                                    ...
                                    <a class="btn btn-sm btn-default rounded-pill"
                                        href="?go=pembayaran_autosave&id_order=<?=base64_encode($row->id)?>">Pembayaran</a>
                                    <?php
                                            }
                                    ?>
...
<script>
            ...
            if (result.isConfirmed) {
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        if (obj.status == true) {
                            document.location="?go=riwayat_autosave";
                        }
                    },
                    complete: function () {
                        loader_close();
    ...
    function konfirmasi_pesanan(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "controller/autosave/konfirmasi_pesanan.php",
                    data: {
                        id: id
                    },
                    type: "POST",
