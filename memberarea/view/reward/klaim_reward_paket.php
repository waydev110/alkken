...
if (isset($_GET["plan_reward"])) {
    $id_plan = $_GET["plan_reward"];
} else {
    $id_plan = $cpj->plan_reward();
}

$member = $cm->detail($session_member_id);
$poin_reward = $cbrp->jumlah_poin_reward($session_member_id, $id_plan);
$data_reward = $obj->index($id_plan);
$plan = $cpj->show($id_plan);
if ($plan) {
    // $syarat_ro = $plan->reward_wajib_ro;
} else {
    redirect('404');
}
$kondisi = true;
// if ($syarat_ro == 1) {
//     $jumlah_poin_ro = $cm->jumlah_poin_ro_reward($session_member_id, $id_plan);
//     if ($jumlah_poin_ro > 0) {
//         $kondisi = true;
//     } else {
//         $kondisi = false;
...
<main class="h-100 has-header">
    ...
    <header class="header position-fixed bg-theme">
        <div class="row">
            ...
            <div class="col align-self-center">
                <h5><?= $title ?> <?= $plan->nama_reward_sponsor ?></h5>
            </div>
                            ...
                            <div class="row">
                                <div class="col align-self-center">
                                    ...
                                    <p class="size-12 text-muted">
                                        Silahkan posting ro <strong><?= $plan->nama_reward ?></strong> terlebih dahulu untuk dapat mengklaim reward anda.
                                    </p>
                ...
                ?>
                    <div class="table-list">
                        ...
                        while ($row = $data_reward->fetch_object()) {
                            $no++;
                            if ($kondisi == true) {
                                $bonus_reward = $cbrp->bonus_reward($session_member_id, $row->id);
                                if ($bonus_reward) {
                                ...
                                $poin_reward->reward_kiri = 0;
                                $poin_reward->reward_kanan = 0;
                                $total_bonus = 0;
                                $status = 'Terkunci';
                                $color = 'danger';
                                $icon = 'fa-lock';
                                ...
                                <div class="card-body">
                                    <div class="row mb-2">
                                        ...
                                        <div class="col-auto align-self-center text-end">
                                            <?php if ($poin_reward->reward_kiri >= $row->poin && $poin_reward->reward_kanan >= $row->poin && $status == 'Pending') {
                                            ?>
                                    ...
                                    <div class="row mb-2 position-relative">
                                        ...
                                        <button class="btn btn-34 bg-white text-dark shadow-sm position-absolute start-50 top-50 translate-middle">
    ...
    <div class="main-container container pt-4 pb-4 mb-2" id="blockFirstForm2" style="display:none">
        <div class="card mb-4">
            <div class="card-body">
                <form action="controller/reward/klaim_reward_paket.php" id="formKlaimReward" method="post">
                    ...
                    <div class="row">
                        <div class="col-12 align-self-center text-end mt-0 d-xs-grid">
                            ...
                            <button type="button" class="btn btn-default btn-block btn-lg rounded-pill px-5 order-xs-first mb-3" id="btnKonfirmasi">KONFIRMASI</button>
                        </div>
                    </div>
                </form>
            </div>
...
<script>
    ...
    function KlaimReward(idreward) {
        if (idreward != "") {
            $.ajax({
                type: 'POST',
                url: 'controller/reward/cek_klaim_reward_paket.php',
                data: {
                    idreward: idreward
                },
