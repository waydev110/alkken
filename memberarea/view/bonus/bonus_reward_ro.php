<?php 
    ...
    $cm = new classMember();
    require_once '../model/classBonusRewardRoSetting.php';
    $obj = new classBonusRewardRoSetting();
    require_once '../model/classBonus.php';
    $cbns = new classBonus();

    $data = $obj->index();
    $member = $cm->detail($session_member_id);
    $cek_last_ro = $cm->cek_last_ro($session_member_id);
?>
...
<main class="h-100 has-header">
                        ...
                        <div class="row">
                            <div class="col align-self-center">
                                ...
                                <p class="size-12 text-muted">
                                    Silahkan posting ro terlebih dahulu untuk dapat melihat poin ro team anda.
                                </p>
        ...
        <div class="row">
            ...
            ?>
                ...
                while($row = $data->fetch_object()){
                    $no++;
                    if($cek_last_ro){
                        $bonus_reward_ro = $cbns->bonus_reward_ro($session_member_id, $row->id);
                        if($bonus_reward_ro){
                            if($bonus_reward_ro->status_transfer == '1'){
                                $status = 'Ditransfer';
                                $color = 'success';
                                $icon = 'fa-check';
                            } else {
                        ...
                        <div class="card-body">
                            <div class="row mb-2">
                                ...
                                <div class="col align-self-center ps-0">
                                    <?php if($member->ro_kiri >= $row->jumlah_kiri && $member->ro_kanan >= $row->jumlah_kanan && $status == 'Pending') {
                                    ?>
                                    ...
                                    <p class="mb-0 text-dark"><span
                                            class="tag bg-<?=$color?> size-10 px-2 py-1 text-white"><strong><i
                                                    class="fas <?=$icon?>"></i> <?=$status?></strong></span> <span
                                            class="size-11"></span> </p>
                                    <?php
                                    } ?>
                                </div>
                                <div class="col text-end ps-0">
                                    <p class="mb-0 text-dark"><strong><?=currency($row->nominal_bonus)?></p>
                                </div>
                            </div>
                                ...
                                <div class="col pe-0">
                                    <div class="form-group form-floating-2">
                                        <div class="form-control pt-3 pb-2 text-left">
                                            <span class="text-danger"><?=currency($member->ro_kiri >= $row->jumlah_kiri ? $row->jumlah_kiri : $member->ro_kiri)?></span> 
                                            dari <?=currency($row->jumlah_kiri)?>
                                        </div>
                                ...
                                <div class="col align-self-center ps-0">
                                    <div class="form-group form-floating-2">
                                        <div class="form-control pt-3 pb-2 text-end">
                                            <span class="text-danger"><?=currency($member->ro_kanan >= $row->jumlah_kanan ? $row->jumlah_kanan : $member->ro_kanan)?></span> 
                                            dari <?=currency($row->jumlah_kanan)?>
                                        </div>
                                        <label class="form-control-label text-end pe-1 end-0 start-auto">Kanan</label>
                                    </div>
                                </div>
                                <button
                                    class="btn btn-34 bg-white text-dark shadow-sm position-absolute start-50 top-50 translate-middle">
                                    <i class="fa-solid fa-gift-simple"></i>
                                </button>
                            </div>
                        </div>
                    </div>
    ...
    <div class="main-container container pt-4 pb-4 mb-2" id="blockFirstForm2" style="display:none">
        <div class="card mb-4">
            <div class="card-body">
                <form action="controller/reward/klaim_reward_ro.php" id="formKlaimReward" method="post">
                ...
                <div class="row">
                    <div class="col-12 text-center mb-2">
                        <div class="alert alert-warning" role="alert">
                            <div class="row">
                                <div class="col">
                                    <h5 class="text-center fw-normal">Anda yakin akan mencairkan Reward? Pencairan tidak dapat dibatalkan!</h5>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    <div class="col-12 text-center mb-4">
                        <h5 class="text-center mb-2">NOMINAL REWARD</h5>
                        <div id="detail"></div>
                    </div>
                </div>
                <div class="row">
...
<script>
    ...
    function KlaimReward(idreward) {
        if(idreward != ""){
            $.ajax({
                type: 'POST',
                url: 'controller/reward/cek_klaim_reward_ro.php',
        ...
        formKlaimReward.on("submit", function (e) {
            ...
            $.ajax({
                ...
                success: function (result) {
                    const obj = JSON.parse(result);
                    if (obj.status == true) {
                        var redirect_url = 'bonus';
                        show_success_html(obj.message, redirect_url);
                    } else {
                        var redirect_url = 'bonus_reward_ro';
