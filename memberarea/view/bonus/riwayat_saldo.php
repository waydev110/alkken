<?php 
    ...
    if(isset($_GET['jenis_wallet'])){
        $jenis_wallet = $_GET['jenis_wallet'];
    } else {
        redirect('404');
    }
?>
        ...
        <div class="p-3 bg-light rounded-15">
            <div class="row">
                <div class="col-4">
                    ...
                    <h2 class="size-14"><?=currency($cbns->riwayat_saldo($jenis_wallet, $session_member_id)->debit)?></h2>
                </div>
                <div class="col-4">
                    <h3 class="size-14">Total Transaksi</h3>
                    <h2 class="size-14"><?=currency($cbns->riwayat_saldo($jenis_wallet, $session_member_id)->kredit)?></h2>
                </div>
                <div class="col-4">
                    <h3 class="size-14">Sisa Saldo</h3>
                    <h2 class="size-14"><?=currency($cbns->riwayat_saldo($jenis_wallet, $session_member_id)->sisa)?></h2>
                </div>
            </div>
        </div>
        <div class="row">
...
<script>
    ...
    function get_saldo(start = 0, type = '', e) {
        ...
        $.ajax({
            type: 'POST',
            url: 'controller/bonus/get_saldo.php',
            data: {
                start: start,
                type: type
            },
