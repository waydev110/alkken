...
class classBonusUpline{
    ...
    {
        $sql = "DELETE FROM mlm_bonus_upline 
    ...
    {
        $sql = "SELECT max FROM mlm_bonus_upline_setting 
    ...
    {
        $sql = "SELECT persentase FROM mlm_bonus_upline_persentase 
                    WHERE id_plan = '$id_plan' 
    ...
    public function create($dari_member, $id_member, $nama_samaran, $nominal, $jenis_bonus, $text_keterangan, $id_kodeaktivasi, $max, $created_at){        
        $c = new classConnection();   
        $sql ="CALL GenerasiUplineWithMax($dari_member, $max)";
        $query = $c->_query($sql);
        $total_record = $query->num_rows;
        if($total_record > 0){
            $sql  = "SELECT * FROM mlm_bonus_upline_setting WHERE id_plan = '$jenis_bonus'";
            $c    = new classConnection();
            ...
            while($row = $query->fetch_object()){
                ...
                if($jenis_bonus >= 200){
                    $keterangan = 'Bonus Titik '.$text_keterangan.' dari '.$nama_samaran.' ('.$id_member.') Level ke-'.$generasi; 
                    ...
                    $sql  = "SELECT * FROM mlm_bonus_upline_persentase WHERE id_plan = '$jenis_bonus' AND generasi = '$generasi'";
                    ...
                    $total_sponsori = $this->sponsori($upline);
                    ...
                    if ($kondisi == true){
                        ...
                        $keterangan = 'Bonus Unilevel '.$text_keterangan.' dari '.$nama_samaran.' ('.$id_member.') Level ke-'.$generasi; 
                        
                        if($this->cek_bonus($upline, $id_kodeaktivasi, $jenis_bonus) == 0){
                            $sql = "INSERT INTO mlm_bonus_upline (
                                    id_member,
                                    nominal,
                                    status_transfer,
                                    dari_member,
    ...
    public function create_rekap($dari_member, $id_member, $nama_samaran, $nominal, $jenis_bonus, $text_keterangan, $id_kodeaktivasi, $max, $created_at, $bulan){        
        ...
        if($total_record > 0){
            ...
            while($row = $query->fetch_object()){
                ...
                if ($kondisi == true){
                    $nominal_bonus = floor($nominal * $persentase/100);
                    $cek_ro = $this->cek_ro($upline, $jenis_bonus, $bulan);
                    if($cek_ro >= 1){
                        $bonus = $this->bonus($upline, $id_kodeaktivasi, $jenis_bonus);
                        if(!$bonus){
                            $sql = "INSERT INTO mlm_bonus_upline (
                                    id_member,
                                    nominal,
