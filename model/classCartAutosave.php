...
class classCartAutosave{
    ...
    public function total_order($id_member){
        $sql  = "SELECT c.id_stokis, c.qty, COALESCE(SUM(p.harga*c.qty),0) AS nominal
    ...
    public function index($id_member, $id_stokis){
        $sql  = "SELECT p.id, c.checked, COALESCE(c.qty, 0) AS qty, p.nama_produk, p.gambar, p.harga, p.nilai_produk, p.qty as qty_produk, p.satuan 
                    FROM mlm_produk p
                    LEFT JOIN mlm_cart_autosave c
                    ON c.id_produk = p.id 
                        AND c.id_member = '$id_member'
                        AND c.id_stokis = '$id_stokis'
                        AND c.status = '0'
                        AND c.deleted_at IS NULL
                    WHERE p.id_produk_jenis = '99'";
        $c    = new classConnection();
        $query  = $c->_query($sql);
        return $query;
    }
    ...
    public function index_checkout($id_member, $id_stokis){
        $sql  = "SELECT c.*, p.nama_produk, p.gambar, p.harga, p.nilai_produk  
                    FROM mlm_cart_autosave c
    ...
    public function create(){
        $sql  = "INSERT INTO mlm_cart_autosave (
                    id_member, 
                    id_stokis, 
                    id_produk, 
                    qty, 
    ...
    public function update(){
        $sql  = "UPDATE mlm_cart_autosave SET
                    qty = '".$this->get_qty()."', 
