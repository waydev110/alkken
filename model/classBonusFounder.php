...
class classBonusFounder{
    ...
    {
        $sql = "UPDATE mlm_bonus_founder s
    ...
    public function index_founder()
    {
        $sql = "SELECT *  
                    FROM mlm_bonus_founder_setting 
                    ORDER BY id ASC";
        $c = new classConnection();
        $query = $c->_query($sql);
        return $query;
    ...
    public function create_binary($dari_member, $id_member, $user_member, $id_founder, $nominal, $jenis_bonus, $keterangan, $id_kodeaktivasi, $created_at){        
        $c = new classConnection();   
        ...
        if($total_record > 0){
            while($row = $query->fetch_object()){
                ...
                $founder = $row->founder;
                if($founder == $id_founder){
                    $keterangan = $keterangan.' dari '.$user_member.' ('.$id_member.') Generasi ke-'.$generasi;                    
                    if($this->cek_bonus($upline, $id_kodeaktivasi, $jenis_bonus) == 0){
                        $sql = "INSERT INTO mlm_bonus_founder (
                                id_member,
                                nominal,
                                status_transfer,
                                dari_member,
