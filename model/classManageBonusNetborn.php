...
class classManageBonusNetborn
{
    ...
    )
    {
        ...
        $sql  = "SELECT 
                    ...
                    GROUP BY sp.id_member) wd ON k.id_member = wd.id_member";
        ...
        while ($row = $query->fetch_object()) {
            ...
            if (in_array('pending', $actions)) {
                $aksi .= '<button type="button" class="btn btn-warning btn-xs" onclick="pending('."'".$row->id."', '".$row->updated_at."', '".$row->total."'".', this)"><i class="fas fa-clock"></i> Pending</button>';
            }
            if (in_array('notif', $actions)) {
                $aksi .= '<button type="button" class="btn btn-info btn-xs" onclick="notif('."'".$row->id."', '".$row->updated_at."', '".$row->total."'".', this)"><i class="fas fa-bell"></i> Notif</button>';
            }
            ...
            if(saldo_bonus($row->id) < $row->total){
                $aksi = '<span class="text-danger fw-bold">Saldo Bonus Kurang</span>';
            }
            $aksi .= '</div>';
